...
let configureScore (world: Container) =
    [ world.On(fun (LoadContent game) ->
          world.Create().With(createScore P1).With(createText game P1) |> ignore

          world.Create().With(createScore P2).With(createText game P2) |> ignore)

      world.On<ScoreIncrease>(
          fun (e: ScoreIncrease) (score: Score) ->
              match e.PlayerIndex with
              | x when x = score.Player -> { score with Value = score.Value + 1uy }
              | _ -> score
          |> Join.update1
          |> Join.over world
      )
